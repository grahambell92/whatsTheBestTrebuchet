...
ranges = []
massRatios = np.logspace(1,4,80)
for m1s in massRatios:
    ranges.append(pendTreb(m1s))
    print(m1s)
...
# Plot 192 m1/m2 and 0.95%
# Vertical line
plt.semilogx([192, 192], [0, 0.95], color='r', linewidth=0.5, linestyle='-')
# Horizontal line
plt.semilogx([10, 192], [0.95, 0.95], color='r', linewidth=0.5, linestyle='-')
plt.semilogx([192], [0.95], 'r.', markersize=2.0)
plt.semilogx(massRatios,ranges, color="k", marker="o", markersize = 2,markerfacecolor='w', linestyle = "None")
ax.set_xlabel(r'CW/projectile ($m_{1}/m_{2}$) mass ratio')
ax.set_ylabel(r'Range/Maximum range')
ylimits = plt.ylim(0.1,1.1)
xlimits = plt.xlim(0, 10000)
...
plt.tight_layout()
plt.show()
savefig('AsymptoticMassRatioTreb')
